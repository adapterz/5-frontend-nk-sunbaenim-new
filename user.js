...
async function request(path, method, body, headers = {}) {
  ...
  let data = "";
  //22.05.12 아래 조건문을 구현한 이유 : 204 상태코드인 경우 res.json() 값이 없어 에러가 뜨기 때문에, 204인 경우 null을 반환하도록 설정
  if (res.status === 204) {
    data = "null";
    return data;
  }
...
function reqCreateNickname() {
  ...
  request(`users/${user_id}/nickname`, httpMethod.post, body)
    .then((data) => console.log(data))
    .catch((error) => console.log(error));
}
...
function reqCreateField() {
  ...
  // 5. Android : 5
  // 6. AI : 6
  request(`users/${user_id}/fields`, httpMethod.post, body)
    .then((data) => console.log(data))
    .catch((error) => console.log(error));
}
...
function reqLogin() {
  ...
  const options = {
    method: "POST",
    //22.05.13 TODO: 아래의 정확한 기능이 뭔지 공부 필요
...
function reqLogout() {
  ...
  fetch(url, options)
    //22.05.12 여기서 바로 console.log(res.json())을 하게 되면 pending이 떠버린다. 왜? 아직 응답 데이터를 받아오지 않았으니까.
...
function reqPwdSearch() {
  ...
  request(`users/pwd_search`, httpMethod.patch, body)
    .then((data) => console.log(data))
    .catch((error) => console.log(error));
}
...
function reqPwdChange() {
  ...
  const options = {
    method: "PATCH",
    //22.05.13 TODO: req.session.user_id값을 서버에 주기 위해 추가 -> 하지만 기능하지 않는다. 왜?
    withCredentials: true,
    headers: {
      "Content-Type": "application/json",
    },
